...
void scribe::startServer() {
  ...
  if (g_Handler->sslOptions->sslIsEnabled()) {
    server.reset(new TThreadedServer(processor, serverSocket, framedTransportFactory, protocol_factory));
  } else {
    server.reset(new TNonblockingServer(processor, protocol_factory, g_Handler->port, thread_manager));
  }
